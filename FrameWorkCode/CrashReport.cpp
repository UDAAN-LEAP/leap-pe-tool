...
namespace CrashReport
{
   ...
   void  _writeLog( const QString &inSignal, const QStringList &inFrameInfoList )
   {
      const QStringList cReportHeader{
         QStringLiteral( "%1 %2 CRASH LOG" ).arg( QCoreApplication::applicationName(), QCoreApplication::applicationVersion()),
      ...
      const QString  cFileName = QStringLiteral( "%1/%2-Crash.log" ).arg( sCrashReportDirPath,
                                                                             QDateTime::currentDateTime().toString( "dd-MM-yy_HHmmss"));
