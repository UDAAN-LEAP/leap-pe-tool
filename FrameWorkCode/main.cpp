...
int main(int argc, char *argv[])
{
    ...
    QFile logFile(QString::fromStdString(qApp->applicationDirPath().toStdString())+"/Crash Logs/application_log.txt");
    logFile.open(QFile::ReadWrite | QFile::Append);
    QTextStream ts(&logFile);
    ts << endl << "OpenOCRCorrect Application Log" << endl << "Date: " <<  QDateTime::currentDateTime().toString( "dd MMM yyyy @ HH:mm:ss" ) << endl << "User : " << QString("Username:" + qgetenv("USERNAME")) << endl << endl << "Application Log:" << endl ;
    ts.setCodec("UTF-8");
    logFile.flush();
    logFile.close();
